...
# Coerce numerics & drop NA
for c in feature_cols + [label_col]:
    df[c] = pd.to_numeric(df[c], errors="coerce")
df = df.dropna().reset_index(drop=True)

# Map labels {-1,1} -> {0,1} if detected
...
do_standardize = st.checkbox("Standardize (StandardScaler)", value=True if model_name=="LogisticRegression" else False)
...
with c1:
    ...
    report_txt = classification_report(y_test, y_pred, target_names=[neg_label_name, pos_label_name], digits=3)
st.text(report_txt)
st.download_button("下載 Classification Report (.txt)", data=text_to_txt_bytes(report_txt), file_name="classification_report.txt", mime="text/plain")
with c2:
    ...
    st.pyplot(fig)
import pandas as pd
cm_df = pd.DataFrame(cm, index=[neg_label_name, pos_label_name], columns=[neg_label_name, pos_label_name])
st.download_button("下載 Confusion Matrix 圖 (PNG)", data=fig_to_png_bytes(fig), file_name="confusion_matrix.png", mime="image/png")
st.download_button("下載 Confusion Matrix 數據 (CSV)", data=df_to_csv_bytes(cm_df.reset_index().rename(columns={"index":"true"})), file_name="confusion_matrix.csv", mime="text/csv")
...
st.download_button("下載 ROC 圖 (PNG)", data=fig_to_png_bytes(fig2), file_name="roc_curve.png", mime="image/png")
st.download_button("下載 ROC 數據 (CSV)", data=df_to_csv_bytes(roc_df), file_name="roc_curve.csv", mime="text/csv")
...
st.download_button("下載 PR 圖 (PNG)", data=fig_to_png_bytes(fig3), file_name="pr_curve.png", mime="image/png")
st.download_button("下載 PR 數據 (CSV)", data=df_to_csv_bytes(pr_df), file_name="pr_curve.csv", mime="text/csv")
...
if model_name == "RandomForest":
    ...
    st.pyplot(fig4)
fi_df = pd.DataFrame({"feature":[str(feature_cols[i]) for i in idx], "importance":importances[idx]})
st.download_button("下載 Feature Importance 圖 (PNG)", data=fig_to_png_bytes(fig4), file_name="feature_importance.png", mime="image/png")
st.download_button("下載 Feature Importance 數據 (CSV)", data=df_to_csv_bytes(fi_df), file_name="feature_importance.csv", mime="text/csv")
    ...
    st.pyplot(fig5)
coef_df = pd.DataFrame({"feature":[str(feature_cols[i]) for i in idx], "coefficient":coefs[idx]})
st.download_button("下載 Coefficients 圖 (PNG)", data=fig_to_png_bytes(fig5), file_name="coefficients.png", mime="image/png")
st.download_button("下載 Coefficients 數據 (CSV)", data=df_to_csv_bytes(coef_df), file_name="coefficients.csv", mime="text/csv")
